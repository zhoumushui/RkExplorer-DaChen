...
import com.android.rockchip.nfs.NfsInfo;
import com.android.rockchip.nfs.NfsController;
import com.android.rockchip.nfs.NfsController.OnNfsListener;
...
public class RockExplorer extends Activity 
	...
	public static final boolean SATA_ENABLED = false;
...
//	public Handler mUIhandler;
	...
	private NfsController mNfsController;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		mNfsController = new NfsController(this);
		mNfsController.setOnNfsListener(nfsListener);
        initData();
        initViews();
    ...
    private OnNfsListener nfsListener = new OnNfsListener() {
    	public void onNfsAdd(NfsInfo info) {
    		String currPath = mFileControl.get_currently_path();
    		//root directory
    		LOG("onNfsAdd currPath " + currPath);
    		if(currPath!=null&&currPath.equals(EnumConstent.mDirSmb)){
        		mCifsProxy.getCifsContent(currPath);
        	}
    	}
    };
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {	
		...
		LOG("onItemSelected->mAnimationEnable:" + mAnimationEnable);
		LOG("onItemSelected->item:" + item);
		...
		LOG("onItemSelected new position=" + position);
    ...
    public void initData(){
		...
		Log.i(TAG, "usb root dir:" + usb_dir);
		sata_dir = StorageUtils.getSataDir();
...
*/
	...
	public boolean cifsIsMountAndConnect(String smbpath){
    	...
    	for (String str : strlist){
			//LOG("smbfileThreadRun cifsIsMountAndConnect  str " + str);
    		String split[] = str.split(" ");
    		if (split.length > 4 && split[4].equals("cifs")){
    			String str1 = split[0].replace("\\"+"040", " ").replace("\\"+"134", "/");
    			//LOG("smbfileThreadRun cifsIsMountAndConnect  str " + str1);
    			cifslist.add(str1);
    		}
    	}
    	if(cifslist.contains(smbpath)){
    		String ip = smbpath.substring(2, smbpath.indexOf("/", 2));
			LOG("smbfileThreadRun ping ip");
    		if(ping(ip))
    			return true;
    		else
    ...
    public void setCurrentFileDir(String content){
    	...
    	} else if((sata_dir != null) && content.startsWith(sata_dir)){		
    		mTextTitle.setText(getString(R.string.str_sata_name) + content.substring(sata_dir.length()));		
...
//				mTextTitle.setText(getString(R.string.str_flash_name)  + content.substring(flash_dir.length()));
    		...
    		mTextTitle.setText(getString(R.string.str_flash_name)  + content.substring(flash_dir.length()));
    	}else if(content.equals(smb_dir) || content.equals(EnumConstent.mDirSmbMoutPoint)){
    	...
    	}else if (content.startsWith(EnumConstent.mDirNfs+"/")){
    		mTextTitle.setText("nfs://"+content.substring(4));
    	...
    	}else if (content.startsWith(EnumConstent.mDirNfsMoutPoint)){
    		Log.d(TAG, "setCurrentDir nfs " + content);
    		String nfspath = content.substring(EnumConstent.mDirNfsMoutPoint.length());
    		if (nfspath.startsWith("/")) {
    			nfspath = nfspath.substring(1);
    		}
    		int indexSplit = nfspath.indexOf("/");
    		if(indexSplit > 0) {
    			nfspath = nfspath.substring(indexSplit);
    		} else {
    			nfspath = "";
    		}
    		String serverAddr = mNfsController.mapToServerAddr(content);
    		mTextTitle.setText("nfs://" + serverAddr + nfspath);
    	}else{
	...
	String ret = "";
		...
		if((usb_dir != null) && content.startsWith(usb_dir)){
            ...
            ret = getString(R.string.str_sata_name) + content.substring(sata_dir.length());
        }else if((sdcard_dir != null) && content.startsWith(sdcard_dir)){
	...
	private OnItemSelectedListener mListItemSelectedListener = new OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
			...
			if(mSelectedPosition >= mFileControl.folder_array.size())
				return;
	...
	public boolean checkPath(String path)
	{
		...
		if((flash_dir != null) && path.startsWith(flash_dir) ||
		        ...
		        (sata_dir != null) && path.startsWith(sata_dir) ||
    ...
    OnItemLongClickListener mListItemLongListener = new OnItemLongClickListener(){
		public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if(!mFileControl.is_first_path){
				if(0 != position || mFileControl.currently_path.equals(smb_dir)){
	...
	public void showSmbEditorDialog(){
        ...
        temp.setOnClickListener(EditorClickListener);
        temp = (View)layout.findViewById(R.id.nfs_new);
        temp.setOnClickListener(EditorClickListener);
		...
		text.setLeft(AutoSize.getInstance().getMargin(0.03f));
		text.setTextSize(TypedValue.COMPLEX_UNIT_PX,AutoSize.getInstance().getTextSize(0.04f));
		text = (TextView)layout.findViewById(R.id.nfs_new_text);		
		text.setLeft(AutoSize.getInstance().getMargin(0.03f));
		text.setTextSize(TypedValue.COMPLEX_UNIT_PX,AutoSize.getInstance().getTextSize(0.04f));
	...
	private View.OnClickListener EditorClickListener = new View.OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.edit_move:		
				...
				if((mSelectedPathList != null) && (mSelectedPathList.size() > 0))
				{
					if(mSelectedPathList.get(0).mFileType != null && mSelectedPathList.get(0).mFileType.equals("..")){
						mEnableMove = false; 
				        mEnableCopy = false;	
					}else{
					    mSourcePathList = mSelectedPathList;
		...
		/*	case R.id.edit_share:
            ...
            case R.id.nfs_new:
            	mNfsController.showAddNfsDialog();
            	mEnableCopy = false;
            	break;
            ...
            case R.id.smb_edit:
            	...
            	fileInfo = mSelectedPathList.get(0);
            	spath = fileInfo.getmFile().getPath();
            	if (spath!=null&&spath.startsWith(EnumConstent.mDirNfs)) {
            		mNfsController.showEditNfsDialog(spath.substring(4), fileInfo.getMMountpoint());
            	} else {
            ...
            case R.id.smb_delete:
            	...
            	spath = fileInfo.getmFile().getPath();
            	if (spath!=null&&spath.startsWith(EnumConstent.mDirNfs)) {
            		String localPath = fileInfo.getMMountpoint();//mNfsController.mapToMountPath(spath);
            		LOG("remove serverPath: " + spath);
            		LOG("remove localPath: " + localPath);
            		mNfsController.removeNfsDevice(localPath);
            		String currPath = mFileControl.get_currently_path();
            		mCifsProxy.getCifsContent(currPath);
            	} else {
	...
	void onShare()
	{
		Log.i(TAG, "onShare->mSelectPathList:" + mSelectedPathList);
		if(mSelectedPathList == null || mSelectedPathList.size() == 0){
			Toast.makeText(this, getString(R.string.empty_directory), Toast.LENGTH_SHORT).show();
			return;
		}
		
		if(mSelectedPathList.size() == 1 && mSelectedPathList.get(0).getmFile().isDirectory()){
			Toast.makeText(this, getString(R.string.empty_directory), Toast.LENGTH_SHORT).show();
			return;
		}
			...
			switch (event.getKeyCode()) {
				case KeyEvent.KEYCODE_BACK:	
					...
					if((mFileControl.get_currently_path().equals(flash_dir) || 
						...
						mFileControl.get_currently_path().equals(EnumConstent.mDirNfs)))
					{
...
//			setAlphaAndTextColor(temp_edit_share_text, false);
    ...
    public boolean openDir(String path){
    	...
    	boolean doInThread = false;
    	if(path.startsWith(EnumConstent.mDirNfs+"/")) {
    		LOG("nfs server " + path);
    		path = mNfsController.mapToMountPath(path.substring(4));
    		if (path == null && mCurListItem != null) {
    			path = mCurListItem.getMMountpoint();
    		}
    		doInThread = true;
    		LOG("nfs local " + path);
    	}
    	...
    	long file_count = doInThread ? 2000 : files.list().length;
...
//                scan_path = usb_dir5;//"usb5"
		...
		LOG("rescanStorage() ->sendBroadcast(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE),rescanFilePath:"+rescanFilePath);
    
        //make sur media scanner can update
        Intent scanIntent = new Intent(Intent.ACTION_MEDIA_MOUNTED);
        scanIntent.setData(Uri.fromFile(new File(rescanFilePath)));
	...
	private void setToolBarText()
	{
		...
		if(mFileControl.currently_path != null){
			if(mFileControl.currently_path.startsWith(smb_dir+"/")
					||mFileControl.currently_path.startsWith(EnumConstent.mDirNfs+"/")){
				multi.setTextColor(Color.GRAY);
				editor.setTextColor(Color.GRAY);
			}
		}
...
//		f.addAction(Intent.ACTION_MEDIA_MOUNTED);
		...
		mCifsProxy.onCreate();
		mCifsProxy.setNfsController(mNfsController);
...
//    	String status = StorageUtils.getSDcardState();
    ...
    /*
     * 閼搭偉鍓婚懘顐ュ姰閼存粏鍔忛棁鑼跺妿绾板苯宕甸懘鐔稿箠閼淬垼鍔呯捄顖氶檮閼搭偉鍔弇ount USB
     * */
...
/*    	return true;*/
	...
	{
		String status = StorageUtils.getSataState();
    ...
    public class smbfileThreadRun implements Runnable{
		...
		public void run() {
			// TODO Auto-generated method stub
			String smbpath = parseMountDirToSmbpath(mCurrnetSmb);
			LOG("smbfileThreadRun run path " + smbpath);
			if(!cifsIsMountAndConnect(smbpath)){
				LOG("smbfileThreadRun run disconect");
				Message msg = new Message();
		    	msg.what = EnumConstent.MSG_DLG_HIDE;
		    	mHandler.sendMessage(msg);
			}else {
