...
public class SmbUtil {
	...
	public static final String SHELL_ROOT = "/data/cifsmanager";
	
	public static final String SHELL_PATH = "/data/cifsmanager/cifsmanager.sh";
	public static final String SHELL_LOG_PATH = "/data/cifsmanager/log";
	...
	public static String mount(String smbpath,FileInfo smbinfo){
		...
		if (!shellLog.exists()){
			try {
				...
				Log.i(TAG, "mount shell log fail");
				return result;
			}
		}
		...
		if(anonymous){
			...
			try
	        {
		        ...
		        if (!ShellFileWrite(cmd)){
		        	allline = res.getString(R.string.shell_error);
		        	Log.i(TAG, "shell file write error 2");
		        	return allline;
		        }	           
	        }
	        catch (Exception ex)
		...
		while(true){
			...
			if(mountFile != null && mountFile.exists() && mountFile.list() != null &&
					mountFile.list().length > 0){
				allline = null;
				break;
			}
			else
				allline = res.getString(R.string.mount_fail);
	...
	public static boolean creatMountPoint(String path){
		...
		if(!new File(abpath).exists()){
			...
			// TODO: handle exception
			Log.i(TAG, "creat mount point exception:" + e);
			e.printStackTrace();
		}
		return true;
